//! crates/guard-cli/src/denial_log.rs
//!
...
pub fn filter_block_destinations(
    ...
    for line in reader.lines() {
        ...
        if v.get("event").and_then(|e| e.as_str()) != Some("block") {
            continue;
        }
        if v.get("run_uuid").and_then(|r| r.as_str()) != Some(run_uuid) {
            continue;
        }
...
mod tests {
    ...
    fn most_recent_returns_last_block_uuid() {
        ...
        assert_eq!(
            most_recent_run_with_denials(&log).unwrap(),
            Some("Z".into())
        );
    }
