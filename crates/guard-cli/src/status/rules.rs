//! crates/guard-cli/src/status/rules.rs
//!
//! `stt-guard status rules [--include-built-in] [--disable <pattern> --reason <reason>] [--enable <pattern>]`.

use std::path::Path;

use guard_ipc::RuleRow;
...
pub fn run(
    ...
    if let Some(pattern) = disable {
        let reason = match reason {
            ...
            _ => {
                eprintln!("stt-guard: --disable requires --reason");
