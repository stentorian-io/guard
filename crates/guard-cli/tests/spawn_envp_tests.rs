...
use guard_cli::ipc_client::{probe_daemon_alive, register_root_with_daemon};
...
fn register_root_with_daemon_round_trips_ack() {
    ...
    assert!(
        r.is_ok(),
        "register_root_with_daemon should return Ok: {r:?}"
    );
    ...
    let node = tree
        .find_node_by_pid(self_pid)
        .expect("node with self_pid must exist");
    assert_eq!(
...
fn probe_daemon_alive_succeeds_against_live_socket() {
    ...
    let h = thread::spawn(move || {
        server
            .accept_one()
            .expect("accept_one tolerates connect+EOF benignly");
    });

    let r = probe_daemon_alive(&sock);
    assert!(
        r.is_ok(),
        "probe should succeed against a live daemon socket: {r:?}"
    );
    ...
    assert_eq!(
        tree.nodes_len(),
        0,
        "probe-only must not record a tracked root"
    );
}
...
fn spawn_wrapped_against_echo_returns_valid_pid() {
    ...
    let pid =
        guard_cli::spawn::spawn_wrapped(prog, &[OsStr::new("hello")], dylib.path(), mfst.path())
...
fn audit_token_for_pid_succeeds_against_spawned_child() {
    ...
    let pid =
        guard_cli::spawn::spawn_wrapped(prog, &[OsStr::new("0.2")], dylib.path(), mfst.path())
            .expect("spawn_wrapped sleep");
    assert!(pid > 0);

    // Sample the audit token while the child is alive.
    let token = guard_cli::audit_token::audit_token_for_pid(pid)
