use guard_core::{
    ...
    has_user_allow, is_cloud_metadata_host, is_cloud_metadata_ip, is_loopback_host, is_loopback_ip,
};
...
fn blocker_01_regression_libc_hot_path_contract() {
    ...
    assert_eq!(
        v1,
        Verdict::Deny,
        "cloud-metadata hard rule must win even with UserAllow override"
    );
