use guard_core::{SCHEMA_V1, SCHEMA_V2, Snapshot};
...
fn v2_default_has_v2_schema_and_nonempty_entries() {
    ...
    assert!(
        s.run_uuid.is_none(),
        "daemon-startup snapshot has no run_uuid"
    );
}
...
fn decode_rejects_v1_schema() {
    ...
    match res {
        Err(guard_core::Error::SchemaVersionMismatch { expected, got }) => {
...
fn decode_truncated_returns_codec_error() {
    ...
    assert!(matches!(res, Err(guard_core::Error::Codec(_))));
}
