//! crates/guard-daemon/src/baseline_staging.rs
//!
//! v0.3 — per-run baseline accumulator.
//!
//! In `stt-guard wrap --baseline` mode, every allow-and-log decision is recorded
...
impl BaselineStaging {
    ...
    pub fn peek_count(&self, run_uuid: &str) -> usize {
        let g = self
            ...
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        g.get(run_uuid).map_or(0, std::vec::Vec::len)
    }
}
