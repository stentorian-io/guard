//! crates/guard-daemon/src/handlers/baseline_commit.rs
//!
//! v0.3 — BaselineCommit handler.
//!
//! Called by `stt-guard wrap --baseline` on tracked-root exit. Consumes the
//! per-run baseline_staging accumulator and returns it to the CLI.

use guard_ipc::{BaselineCommit, BaselineCommitReply};
...
pub fn handle_baseline_commit(req: &BaselineCommit, state: &DaemonState) -> BaselineCommitReply {
    let _run = match state.process_tree.get_run(&req.run_uuid) {
        Some(r) => r,
        None => return BaselineCommitReply::err(format!("no run record for {}", req.run_uuid)),
    };
