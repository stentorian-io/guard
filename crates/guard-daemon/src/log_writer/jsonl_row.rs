//! crates/guard-daemon/src/log_writer/jsonl_row.rs
//!
...
pub fn truncate_argv(mut argv: Vec<String>) -> Vec<String> {
    ...
    argv.into_iter()
        .map(|s| truncate_str(s, MAX_ARGV_BYTES))
        .collect()
}
...
mod tests {
    ...
    fn root() -> RootCtxLog {
        RootCtxLog {
            audit_token: [0; 8],
            argv: vec!["stt-guard".into(), "run".into()],
        }
    }
    ...
    fn block_row_serializes_with_event_block() {
        ...
        assert!(
            !s.contains("\"intel\""),
            "intel field should be omitted when None: {s}"
        );
        assert!(
    ...
    fn now_rfc3339_lexicographic_ordering() {
        ...
        assert!(
            t1 <= t2,
            "RFC3339 strings not monotonically ordered: {t1} vs {t2}"
        );
