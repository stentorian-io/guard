//! crates/guard-daemon/src/log_writer/mod.rs
//!
...
pub use jsonl_row::{
    Decision, GapRecord, JSONL_SCHEMA_VERSION, LogRow, ProcessCtxLog, RootCtxLog, now_rfc3339,
    truncate_argv,
};
...
impl LogWriter {
        ...
        std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                while let Ok(row) = rx.recv() {
                    match &row {
                        LogRow::Block(_) => {
                            blocks_t.fetch_add(1, Ordering::Relaxed);
                        }
                        LogRow::Allow(_) => {
                            allows_t.fetch_add(1, Ordering::Relaxed);
                        }
                        LogRow::Gap(_) => {
                            gaps_t.fetch_add(1, Ordering::Relaxed);
                        }
                    }
                    ...
                    if rotation::should_rotate(&log_path) {
                        ...
                        match open_active(&log_path) {
                            Ok(f) => active = f,
                            Err(e) => {
                                tracing::error!(error = %e, "reopen post-rotate failed");
                                break;
                            }
                        }
                    }
    ...
    pub fn send(&self, row: LogRow) {
        ...
        // `stt-guard approve --from-log` accumulate entries in memory).
