//! crates/guard-daemon/src/log_writer/rotation.rs
//!
...
pub fn rotate(active_path: &Path) -> io::Result<()> {
    let parent = active_path
        .parent()
        .ok_or_else(|| io::Error::other("active log has no parent"))?;
    ...
    if active_path.exists() {
        ...
        while final_rotated.exists() && bump < 64 {
            ...
            final_rotated = parent.join(format!("{ROTATED_GLOB_PREFIX}{stamp}-{alt_seq:03}.log"));
        }
...
pub fn enforce_retention(dir: &Path) -> io::Result<()> {
    ...
    if let Ok(entries) = fs::read_dir(dir) {
        for entry in entries.flatten() {
            ...
            if !name.starts_with(ROTATED_GLOB_PREFIX) || !name.ends_with(ROTATED_GLOB_SUFFIX_GZ) {
                continue;
            }
            let path = entry.path();
