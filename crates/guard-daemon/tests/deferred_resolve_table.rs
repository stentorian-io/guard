...
use guard_daemon::daemon_state::{DeferredEntry, DeferredResolveTable};
...
fn entry(
    ...
    DeferredEntry,
    crossbeam_channel::Receiver<guard_daemon::policy::Verdict>,
) {
    let (tx, rx) = bounded(1);
    (
        DeferredEntry {
...
fn insert_and_take_round_trip() {
    ...
    let _ = sender.send(guard_daemon::policy::Verdict::Allow);
    assert!(matches!(
        rx.recv().unwrap(),
        guard_daemon::policy::Verdict::Allow
    ));
...
fn drain_for_run_signals_deny_and_removes() {
    ...
    for rx in &a_rxs {
        assert!(matches!(
            rx.recv().unwrap(),
            guard_daemon::policy::Verdict::Deny
        ));
    }
