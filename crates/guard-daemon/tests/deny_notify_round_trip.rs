...
use guard_ipc::{AuditTokenWire, DenyNotify, DenyNotifyAck, IPC_SCHEMA_V4, RegisterRoot, Reply};
...
fn send_deny_notify_and_recv_ack(stream: &mut UnixStream, msg: &DenyNotify) -> DenyNotifyAck {
...
fn deny_notify_round_trip_logs_block_row() {
    ...
    let log_path = tmp.path().join("stt-guard.log");
    ...
    let mut state = DaemonState::new(tree.clone(), det, rs, curated, tmp.path().to_path_buf());
...
fn deny_notify_untracked_sender_still_logs() {
    ...
    let mut state = DaemonState::new(tree, det, rs, curated, tmp.path().to_path_buf());
