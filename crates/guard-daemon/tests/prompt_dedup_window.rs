use guard_daemon::prompt::{CoalesceOutcome, PromptDedup};
...
fn different_port_is_fresh_d46_port_granularity() {
    ...
    d.coalesce_with_now(now, "r1", "h", 443, "p1");
    assert_eq!(
        d.coalesce_with_now(now, "r1", "h", 80, "p2"),
        CoalesceOutcome::Fresh
    );
}
...
fn different_run_uuid_is_fresh() {
    ...
    d.coalesce_with_now(now, "r1", "h", 443, "p1");
    assert_eq!(
        d.coalesce_with_now(now, "r2", "h", 443, "p2"),
        CoalesceOutcome::Fresh
    );
}
...
fn after_5_seconds_window_is_fresh_again() {
    ...
    assert_eq!(
        d.coalesce_with_now(after, "r1", "h", 443, "p2"),
        CoalesceOutcome::Fresh
    );
}
...
fn forget_removes_entry() {
    ...
    d.forget("r1", "h", 443);
    assert_eq!(
        d.coalesce_with_now(now, "r1", "h", 443, "p2"),
        CoalesceOutcome::Fresh
    );
}
