use guard_daemon::prompt::generate_suggested_rules;

#[test]
fn s3_amazonaws_shared_cdn_three_suggestions() {
    let s = generate_suggested_rules("foo.s3.amazonaws.com");
    assert_eq!(s.len(), 3, "got: {s:?}");
    assert_eq!(s[0].pattern, "foo.s3.amazonaws.com");
    assert_eq!(s[0].match_type, "exact");
    assert_eq!(s[1].pattern, "s3.amazonaws.com");
    assert_eq!(s[1].match_type, "exact");
    assert_eq!(s[2].pattern, ".s3.amazonaws.com");
    assert_eq!(s[2].match_type, "suffix");
}
...
fn workers_dev_not_in_shared_cdn_list() {
    ...
    assert!(
        !s.iter()
            .any(|r| r.pattern == "workers.dev" && r.match_type == "exact")
    );
}
