...
use guard_core::policy::evaluate_policy;
use guard_daemon::handlers::resolve::{handle_resolve, load_run_entries};
use guard_ipc::ResolveReply;
...
fn policy_gate_allows_matching_host() {
    ...
    let (verdict, _source) = evaluate_policy(b"registry.npmjs.org", None, false, &entries);
...
fn policy_gate_denies_unknown_host() {
    ...
    let (verdict, _source) = evaluate_policy(b"evil.attacker.com", None, false, &entries);
