...
//!   2 — operation failed with EHOSTUNREACH (Stentorian Guard denied)
//!   3 — unexpected error
//!   4 — usage error

#[cfg(not(target_os = "macos"))]
fn main() {
    eprintln!("expanded_hooks_probe is macOS-only");
    std::process::exit(4);
}
...
mod macos_probe {
    ...
    pub fn main() {
        ...
        let deny_host =
            std::env::var("STT_GUARD_TEST_DENY_HOST").unwrap_or_else(|_| "discord.com".into());
        let deny_port: u16 = std::env::var("STT_GUARD_TEST_DENY_PORT")
            .ok()
