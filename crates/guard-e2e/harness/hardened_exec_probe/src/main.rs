...
fn main() {
    eprintln!("hardened_exec_probe is macOS-only");
    std::process::exit(4);
}
...
mod macos_probe {
    ...
    fn test_exec_synthetic_fat() {
        let path = write_executable_fixture("guard-fat", &fat_macho_fixture());
    ...
    fn test_exec_synthetic_syscall() {
        let path = write_executable_fixture("guard-syscall", &thin_syscall_fixture());
        let outcome = exec_fixture(&path);
        if outcome == ExecOutcome::Errno(libc::EACCES) {
