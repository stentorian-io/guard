...
impl DaemonHarness {
    ...
    pub fn start_with_env_and_home_setup(
        ...
        // e.g. /tmp/.se2eXXXXXX/stt-guard-daemon.sock => ~32 bytes (well under limit).
...
pub fn read_pty_until(
    ...
    std::thread::Builder::new()
        .name("guard-e2e-pty-reader".into())
        .spawn(move || {
...
fn spawn_daemon_into(
    ...
    let logs = home_path.join("Library/Logs/Stentorian Guard");
    std::fs::create_dir_all(&logs)?;

    let daemon_bin = cargo_target_dir().join("stt-guard-daemon");
    if !daemon_bin.exists() {
        return Err(std::io::Error::other(format!(
            "stt-guard-daemon binary not found at {} — run cargo build first",
            daemon_bin.display()
        )));
    }
    ...
    let ready = guard_daemon::state_dir::ready_path(state_dir);
    ...
    let socket = guard_daemon::state_dir::socket_path(state_dir);
    let manifest = guard_daemon::state_dir::manifest_path(state_dir);
