...
//!
//! Per CONTEXT D-02 triple-defense (2): even if Stentorian Guard fails to block, the
...
/// Stentorian Guard returned by `start_or_hosts`. Holds whichever variant fired so the
...
pub fn start_or_hosts(hosts: &[&str], fallback_port: u16) -> io::Result<SinkGuard> {
    match HostsRewriter::new(hosts) {
        ...
        Err(e) => {
            eprintln!(
                "[sink_listener] /etc/hosts rewrite failed ({e}); falling back to localhost listener"
            );
