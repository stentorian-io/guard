...
fn open_prompt_channel_init(
    ...
    write_frame(&mut s, &init).map_err(|e| std::io::Error::other(format!("frame write: {e}")))?;
    ...
    let ack: PromptChannelInitAck =
        read_frame(&mut s).map_err(|e| std::io::Error::other(format!("read ack: {e}")))?;
    Ok((s, ack))
}
...
fn start_background_tracked_run(
    ...
    let child = std::process::Command::new(&cli)
        ...
        .spawn()
        .expect("spawn background stt-guard wrap");
...
fn sixty_fifth_prompt_channel_init_is_err_acked_with_cap_message() {
    ...
    let sock = guard_daemon::state_dir::socket_path(&harness.state_dir);
    ...
    assert_eq!(
        held.len(),
        64,
        "all 64 channels must Ok-Ack before cap test"
    );
