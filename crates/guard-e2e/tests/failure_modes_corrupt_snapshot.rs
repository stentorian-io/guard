...
//! `STT_GUARD_SNAPSHOT_MANIFEST=$corrupt_manifest`. We BYPASS `stt-guard wrap`
...
fn corrupt_snapshot_causes_dylib_to_fail_closed() {
    ...
    // BYPASS `stt-guard wrap` per RESEARCH §A1 — spawn_wrapped strips and
    // re-sets STT_GUARD_SNAPSHOT_MANIFEST, so we cannot inject the corrupt
    // manifest through the CLI orchestrator.
    //
    // STT_GUARD_STATE_DIR is set so the dylib's `well_known_state_dir()` path
    ...
    // Stentorian Guard HOME-derivation ($HOME/Library/Application Support/Stentorian Guard) and reject
    ...
    //
    // The dylib loads at node's ctor → reads STT_GUARD_SNAPSHOT_MANIFEST →
    ...
    let out = Command::new(&node)
        ...
        .env("STT_GUARD_SNAPSHOT_MANIFEST", &manifest_path)
        .env("STT_GUARD_STATE_DIR", &harness.state_dir)
        // Intentionally NOT setting STT_GUARD_DAEMON_SOCKET — the dylib's
    ...
    let is_fail_closed = fail_closed_codes.iter().any(|c| denied_line.contains(c));
