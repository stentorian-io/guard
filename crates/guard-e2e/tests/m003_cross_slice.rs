...
//!   - S05: `stt-guard status persistence` CLI surface
...
fn lockfile_registries_appear_in_snapshot() {
    ...
    let output = Command::new(&cli)
        ...
        .output()
        .expect("run stt-guard echo in project dir");
    ...
    assert!(
        output.status.success(),
        "stt-guard echo should succeed; stdout={stdout}\nstderr={stderr}"
    );
...
"#;
    ...
    let output = Command::new(&cli)
        ...
        .current_dir(&project_dir)
        .output()
        .expect("run stt-guard echo in cargo project dir");

    assert!(output.status.success(), "stt-guard echo should succeed");
...
fn persistence_write_detected_in_log() {
    ...
    let log = home.join("Library/Logs/Stentorian Guard/stt-guard.log");
...
fn status_persistence_shows_events() {
    ...
    let status_output = Command::new(&cli)
        ...
        .env("PATH", std::env::var_os("PATH").unwrap_or_default())
        .output()
        .expect("run stt-guard status persistence");

    let status_stdout = String::from_utf8_lossy(&status_output.stdout);
    assert!(
        status_output.status.success(),
        "stt-guard status persistence should succeed; stdout={status_stdout}"
    );
    assert!(
        !status_stdout.trim().is_empty(),
        "stt-guard status persistence output should be non-empty"
    );
}
