...
use guard_daemon::snapshot_gc::gc_sweep;
use guard_daemon::state_dir::{ensure_runs_dir, run_manifest_path, run_snapshot_path};
use guard_daemon::tracked::{ProcessTree, RunRecord};
use std::sync::Arc;
...
fn gc_skips_runrecord_with_zero_pid_placeholder() {
    ...
    assert!(
        snap_path.exists(),
        "pending snapshot must NOT be GC'd (zero-pid placeholder)"
    );
