...
fn sigint_during_prompt_sends_cancel_and_propagates_to_child() {
    ...
    // Wait for the prompt to appear (stt-guard wrap's render loop prints "Choose: [1]").
    ...
    // Send SIGINT to the stt-guard wrap process (which will propagate to the pgid).
    // The SIGINT handler (D-79) should: cancel in-flight prompts + SIGINT to pgid.
    unsafe {
        libc::kill(pid, libc::SIGINT);
    }

    // Wait for stt-guard wrap to exit (SIGINT handled + child reaped).
    let _ = child.wait();
