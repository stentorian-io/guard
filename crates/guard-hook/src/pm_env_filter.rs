...
const MAX_PM_ENV_BYTES: usize = guard_ipc::ExecEvent::MAX_PM_ENV_BYTES;
...
pub fn filter_one(key: &str, value: &str) -> Option<(String, String)> {
    ...
    Some((
        key.to_string(),
        env_filter::truncate_value(value).to_string(),
    ))
}
...
/// has to reject an over-size payload from a Stentorian Guard-owned dylib.
///
/// # Safety
...
mod tests {
    ...
    fn drops_password_substring() {
        let (_owners, ptrs) =
            make_envp(&["npm_package_name=lodash", "npm_password_setting=DECOY_PASS"]);
        let out = unsafe { extract_pm_env_from_envp(ptrs.as_ptr()) };
        assert_eq!(out.len(), 1);
        assert_eq!(out[0].0, "npm_package_name");
    ...
    fn cargo_pkg_name_admitted() {
        let (_owners, ptrs) = make_envp(&["CARGO_PKG_NAME=guard-hook"]);
        let out = unsafe { extract_pm_env_from_envp(ptrs.as_ptr()) };
        assert_eq!(out.len(), 1);
        assert_eq!(out[0], ("CARGO_PKG_NAME".into(), "guard-hook".into()));
    }
