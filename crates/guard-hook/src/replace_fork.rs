...
/// closed — the dylib is loaded into a process outside `stt-guard wrap`, so
...
pub unsafe extern "C" fn guard_fork() -> libc::pid_t {
    ...
    // tracked tree (BLOCKER-02 — dylib loaded into a non-`stt-guard wrap`
...
pub unsafe extern "C" fn guard_posix_spawn(
    ...
    //
    // The fork hook (`guard_fork`) and vfork hook (`guard_vfork`)
    ...
    // process Stentorian Guard doesn't actually own).
    //
    ...
    match send_fork_event_sync(parent, new_pid as i32, pv, IPC_TIMEOUT_MS) {
        ...
        Err(e) if is_untracked_peer(&e) => {
            ...
            // under `stt-guard wrap`. Skip the exec-half best-effort IPC too
...
pub unsafe extern "C" fn guard_posix_spawnp(
    ...
    // BLOCKER-05: see `guard_posix_spawn` for the IN_HOOK-thread-local
    ...
    match send_fork_event_sync(parent, new_pid as i32, pv, IPC_TIMEOUT_MS) {
        ...
        Err(_) => {
            ...
            // kill. See guard_posix_spawn for the full discussion.
            let now_pv = child_pidversion(new_pid);
            if now_pv != 0 && now_pv == saved_pv {
                unsafe {
                    libc::kill(new_pid, libc::SIGKILL);
    ...
    //
    // BLOCKER #1 closure (quick-260508-et9): walk the caller-supplied envp
    // and filter to PM-relevant keys. Same contract as guard_posix_spawn.
