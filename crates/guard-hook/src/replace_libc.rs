...
/// V2 tier-walk evaluator (`guard_core::policy::evaluate_policy`) — the
...
#[unsafe(no_mangle)]
unsafe extern "C" fn guard_connectx(
    s: c_int,
...
// it globally, so REAL_GETADDRINFO pointed back to guard_getaddrinfo
...
fn decide_for_sockaddr(addr: *const sockaddr, addrlen: socklen_t) -> (Verdict, SourceKind) {
    ...
    let host = if host.is_none() && daemon_socket_path().is_some() {
        ...
        if is_inet && not_loopback && not_imds {
            ...
            let mut host_from_resolve: Option<(
                /* buf */ [u8; MAX_HOSTNAME],
                /* len */ usize,
            )> = None;
...
unsafe extern "C" {
    ...
    fn send(s: c_int, buf: *const c_void, len: size_t, flags: c_int) -> ssize_t;
}
...
static STT_GUARD_INTERPOSE_CONNECT: [SyncPtr; 2] = [
    SyncPtr(guard_connect as *const c_void),
...
static STT_GUARD_INTERPOSE_CONNECTX: [SyncPtr; 2] = [
    SyncPtr(guard_connectx as *const c_void),
...
static STT_GUARD_INTERPOSE_SENDTO: [SyncPtr; 2] = [
    SyncPtr(guard_sendto as *const c_void),
...
static STT_GUARD_INTERPOSE_SENDMSG: [SyncPtr; 2] = [
    SyncPtr(guard_sendmsg as *const c_void),
...
static STT_GUARD_INTERPOSE_SEND: [SyncPtr; 2] = [
    SyncPtr(guard_send as *const c_void),
...
static STT_GUARD_INTERPOSE_WRITE: [SyncPtr; 2] = [
    SyncPtr(guard_write as *const c_void),
...
static STT_GUARD_INTERPOSE_WRITEV: [SyncPtr; 2] = [
    SyncPtr(guard_writev as *const c_void),
...
static STT_GUARD_INTERPOSE_GETADDRINFO: [SyncPtr; 2] = [
    SyncPtr(guard_getaddrinfo as *const c_void),
...
static STT_GUARD_INTERPOSE_FREEADDRINFO: [SyncPtr; 2] = [
    SyncPtr(guard_freeaddrinfo as *const c_void),
    SyncPtr(libc::freeaddrinfo as *const c_void),
];

// libc syscall() interpose: DEFERRED.
//
...
// guard_connect. Unknown native binaries containing raw syscall instruction
// bytes are classified T3 by the exec-time scanner and fail closed before
// child creation.
