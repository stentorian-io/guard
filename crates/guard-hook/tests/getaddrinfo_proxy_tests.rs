...
use guard_hook::ipc_client::{_clear_daemon_socket_for_test, _set_daemon_socket_for_test};
...
fn spawn_stub_resolver(reply: ResolveReply) -> (TempDir, PathBuf) {
    ...
    thread::spawn(move || {
        if let Ok((mut stream, _)) = listener.accept() {
            ...
            let _req: guard_ipc::Resolve = read_frame(&mut stream).expect("read Resolve");
...
// Helper: call guard_getaddrinfo directly via its C ABI symbol.
unsafe extern "C" {
    ...
    fn guard_freeaddrinfo(res: *mut libc::addrinfo);
}
...
fn getaddrinfo_proxy_returns_v4_address() {
    ...
    // Ensure FAIL_CLOSED is false and ALLOWLIST is set (guard_getaddrinfo
    // doesn't check policy — it just proxies DNS — but the hook ctor might
    // not have run in this test binary).
    guard_hook::snapshot::FAIL_CLOSED.store(false, std::sync::atomic::Ordering::Release);
    ...
    assert_eq!(rc, 0, "guard_getaddrinfo should return 0 on success");
    ...
    // Free the list.
    unsafe { guard_freeaddrinfo(result) };
}
...
fn getaddrinfo_proxy_returns_eai_fail_on_deny() {
    let _lock = SOCKET_TEST_LOCK.lock().unwrap();
    let reply = ResolveReply::deny("policy: blocked by stt-guard");
    let (_dir, sock_path) = spawn_stub_resolver(reply);
    _set_daemon_socket_for_test(sock_path);
    guard_hook::snapshot::FAIL_CLOSED.store(false, std::sync::atomic::Ordering::Release);
...
fn getaddrinfo_returns_eai_fail_when_fail_closed() {
    ...
    let (_dir, sock_path) = spawn_stub_resolver(reply);
    _set_daemon_socket_for_test(sock_path);
    guard_hook::snapshot::FAIL_CLOSED.store(true, std::sync::atomic::Ordering::Release);
