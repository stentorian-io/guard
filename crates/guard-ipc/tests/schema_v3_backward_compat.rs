...
use guard_ipc::{AuditTokenWire, ExecEvent, IPC_SCHEMA_V2, IPC_SCHEMA_V3, PrepareSnapshot};
...
fn v2_exec_event_decodes_under_v3_struct() {
    ...
    assert!(
        decoded.pm_env.is_empty(),
        "pm_env must default to empty for V2 message"
    );
}
...
fn v3_exec_event_round_trip_preserves_pm_env_order() {
    ...
    let original = ExecEvent::new_v3(
        AuditTokenWire { val: [0u32; 8] },
    ...
    assert_eq!(
        decoded.pm_env, pm,
        "pm_env order must be preserved (Vec, not HashMap)"
    );
    assert_eq!(decoded.schema_version, IPC_SCHEMA_V3);
}
