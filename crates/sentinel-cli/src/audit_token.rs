...
pub fn audit_token_for_pid(pid: libc::pid_t) -> std::io::Result<AuditToken> {
    ...
    if kr == KERN_SUCCESS {
        ...
        tracing::warn!(pid, kr, "task_name_for_pid failed, using proc_pidinfo fallback");
    }
...
fn audit_token_for_pid_fallback(pid: libc::pid_t) -> std::io::Result<AuditToken> {
    ...
    let pidversion_analog: u32 =
        (info.pbi_start_tvsec as u32) ^ (info.pbi_start_tvusec as u32);
