...
fn extract_pip_registries(content: &str) -> BTreeSet<String> {
    ...
    if let Some(meta) = v.get("_meta").and_then(|m| m.get("sources")) {
        if let serde_json::Value::Array(sources) = meta {
            for src in sources {
...
"#;
    ...
    fn discover_lockfile_stops_at_git() {
        ...
        assert_eq!(found.as_deref(), Some(dir.path().join("package-lock.json").as_path()));
    }
}
