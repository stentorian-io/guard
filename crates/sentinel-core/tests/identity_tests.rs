use sentinel_core::identity::{AuditToken, ProcessIdentity, audit_token_to_pid};

#[test]
fn unverified_returns_none_as_policy_key() {
    let id = ProcessIdentity::from_pid_unverified(42);
    assert!(id.as_policy_key().is_none(), "Unverified must not serve as policy key (ENF-08)");
}
...
fn verified_returns_some_as_policy_key() {
    ...
    let k = id.as_policy_key().expect("Verified must yield a policy key");
...
fn pid_accessor_works_for_both_variants() {
    let v_id = unsafe { ProcessIdentity::from_kernel_token(AuditToken::synthetic([0,0,0,0,0,9001,0,0])) };
...
fn audit_token_to_pid_agrees_with_val_5() {
    ...
    assert_eq!(p, 6789, "audit_token_to_pid must read val[5] (knight.sc layout)");
}
