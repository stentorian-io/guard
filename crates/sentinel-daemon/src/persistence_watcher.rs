...
use crate::log_writer::{GapRecord, LogRow, LogWriter, ProcessCtxLog, JSONL_SCHEMA_VERSION};
...
const THREAD_NAME: &str = "sentineld-persist-watch";
...
fn run_watcher(process_tree: Arc<ProcessTree>, log_writer: LogWriter) -> std::io::Result<()> {
    ...
    info!(
        dirs = watched.len(),
        "persistence watcher started"
    );

    let mut fd_to_idx: HashMap<RawFd, usize> = watched
        .iter()
        .enumerate()
        .map(|(i, w)| (w.fd, i))
        .collect();
