...
use sentinel_daemon::curated::{CuratedError, MIN_SUFFIX_LEN, load_curated, parse_yaml};
...
fn includes_required_ecosystems() {
    ...
    assert!(patterns.contains(&".workers.dev"), "ALLOW-06 cloudflare workers");
    assert!(patterns.contains(&".pages.dev"), "ALLOW-06 cloudflare pages");
