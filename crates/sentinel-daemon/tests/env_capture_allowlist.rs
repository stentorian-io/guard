...
fn denylist_blocks_secret_keys() {
    let env = vec![
        ...
        pair("PIP_INDEX_URL", "https://user:pass@pypi.example.com/simple/"),
    ...
    let captured_vals: String = out.iter().map(|(_, v)| v.as_str()).collect::<Vec<_>>().join("|");
...
fn enforces_total_wire_cap() {
    ...
    assert!(total <= sentinel_ipc::ExecEvent::MAX_PM_ENV_BYTES, "total {total} > cap");
    assert!(out.len() < 200, "cap should kick in well before 200 entries");
}
...
fn truncates_oversized_value() {
    ...
    let out = extract_pm_env(&env);
    assert_eq!(out.len(), 1);
    assert!(out[0].1.len() <= 512, "value not truncated to 512 bytes; got {}", out[0].1.len());
}
