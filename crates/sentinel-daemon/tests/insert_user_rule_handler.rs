use sentinel_daemon::handlers::insert_user_rule::handle_insert_user_rule;
use sentinel_daemon::rule_store::RuleStore;
use sentinel_ipc::{IPC_SCHEMA_V3, InsertUserRule, InsertUserRuleReply};
...
fn happy_path_returns_rule_id() {
    ...
    let store = RuleStore::open(&dir.path().join("sentinel.db")).expect("open");
...
fn rejects_bad_kind() {
    ...
    let store = RuleStore::open(&dir.path().join("sentinel.db")).unwrap();
