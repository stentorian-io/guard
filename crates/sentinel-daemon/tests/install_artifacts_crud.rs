...
fn insert_then_list_round_trip() {
    ...
    store.insert("launchagent", "/tmp/agent.plist", Some("abc"), "0.3.0").expect("insert");
    ...
    assert_eq!(rows[0].sentinel_version, "0.3.0");
}
...
fn insert_or_replace_idempotent() {
    let (_dir, db) = fresh_db();
    let store = InstallArtifactStore::open(&db).expect("open");
    store.insert("init_script", "/tmp/init.sh", Some("v1hash"), "0.3.0").expect("first");
    store.insert("init_script", "/tmp/init.sh", Some("v2hash"), "0.3.1").expect("replace");
    let rows = store.list_all().expect("list");
    assert_eq!(rows.len(), 1, "INSERT OR REPLACE collapses on PK");
    assert_eq!(rows[0].content_hash.as_deref(), Some("v2hash"));
    assert_eq!(rows[0].sentinel_version, "0.3.1");
}
...
fn delete_by_pk() {
    let (_dir, db) = fresh_db();
    let store = InstallArtifactStore::open(&db).expect("open");
    store.insert("marker_block", "/tmp/.zshrc", None, "0.3.0").expect("insert");
...
fn delete_all_clears_table() {
    let (_dir, db) = fresh_db();
    let store = InstallArtifactStore::open(&db).expect("open");
    store.insert("launchagent", "/tmp/a.plist", None, "0.3.0").unwrap();
    store.insert("init_script", "/tmp/init.sh", None, "0.3.0").unwrap();
...
fn check_constraint_rejects_bogus_kind() {
    let (_dir, db) = fresh_db();
    let store = InstallArtifactStore::open(&db).expect("open");
    let err = store.insert("not_a_real_kind", "/tmp/x", None, "0.3.0").unwrap_err();
...
fn read_via_db_works_when_no_handle_open() {
    ...
    {
        let store = InstallArtifactStore::open(&db).expect("open");
        store.insert("log_dir", "/tmp/logs", None, "0.3.0").expect("insert");
