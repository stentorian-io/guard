use sentinel_daemon::log_writer::rotation::rotate;
...
fn rotate_does_not_lose_lines_mid_write() {
    ...
    while std::time::Instant::now() < deadline {
        ...
        for e in &entries {
            ...
            if n.starts_with("sentinel-") && n != "sentinel.log" {
                // Either ".log" or ".log.gz" — both acceptable.
                found_rotated = true;
            }
        }
        if found_rotated { break; }
        std::thread::sleep(std::time::Duration::from_millis(50));
    }
    assert!(found_rotated, "rotated artifact missing after rename");
    assert!(active.exists(), "active log missing after post-rotate write");
}
