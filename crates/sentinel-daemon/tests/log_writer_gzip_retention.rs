use sentinel_daemon::log_writer::rotation::{enforce_retention, MAX_ARCHIVES, MAX_TOTAL_BYTES};
...
fn retention_count_keeps_newest_seven() {
    ...
    for i in 0..10 {
        let p = dir.path().join(format!("sentinel-20260101-{:03}.log.gz", i));
        touch(&p, 10);
    ...
    let names: Vec<_> = remaining.iter().map(|e| e.file_name().to_string_lossy().into_owned()).collect();
...
fn retention_size_cap_evicts_oldest() {
    ...
    for i in 0..6 {
        let p = dir.path().join(format!("sentinel-20260102-{:03}.log.gz", i));
        touch(&p, big);
        std::thread::sleep(std::time::Duration::from_millis(15));
    }
    enforce_retention(dir.path()).expect("retention");
    let remaining: u64 = std::fs::read_dir(dir.path()).unwrap()
        .filter_map(|e| e.ok())
        .map(|e| e.metadata().map(|m| m.len()).unwrap_or(0))
        .sum();
    assert!(remaining <= MAX_TOTAL_BYTES, "remaining {remaining} > cap {MAX_TOTAL_BYTES}");
}
