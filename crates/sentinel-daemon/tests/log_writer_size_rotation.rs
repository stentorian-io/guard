use sentinel_daemon::log_writer::rotation::{rotate, should_rotate, SIZE_THRESHOLD};

#[test]
fn should_rotate_at_threshold() {
    let dir = tempfile::tempdir().expect("tempdir");
    let active = dir.path().join("sentinel.log");
...
fn rotate_renames_active_atomically() {
    ...
    assert!(!active.exists() || std::fs::metadata(&active).map(|m| m.len()).unwrap_or(0) == 0,
            "active path either absent or empty after rotate");
    // Within 5s, expect a sentinel-YYYYMMDD-001.log OR .log.gz to exist.
    ...
    while std::time::Instant::now() < deadline {
        let count = std::fs::read_dir(dir.path()).unwrap()
            ...
            .filter(|e| {
                ...
                n.starts_with("sentinel-") && (n.ends_with(".log") || n.ends_with(".log.gz"))
            })
            .count();
