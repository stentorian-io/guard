use sentinel_daemon::os_ffi::{
    csops_status, has_hardened_bits, is_hardened_runtime,
    CS_HARD, CS_OPS_STATUS, CS_REQUIRE_LV, CS_RESTRICT, CS_RUNTIME, SYS_CSOPS,
};
...
fn is_hardened_runtime_on_self_is_false_for_unhardened_test_binary() {
    ...
    assert!(!is_hardened_runtime(pid),
