//! Phase 3 plan 03-04: ProcessTree extensions for is_tty/baseline_mode/project_toml/prompt_channel.
...
fn set_and_get_prompt_channel() {
    ...
    let (tx, rx) = bounded::<sentinel_ipc::PromptRequest>(1);
    tree.set_prompt_channel("run-1", tx);
    assert!(tree.get_prompt_channel("run-1").is_some());
    let taken = tree.take_prompt_channel("run-1");
    assert!(taken.is_some());
    assert!(tree.get_prompt_channel("run-1").is_none(), "take_prompt_channel removes the entry");
    drop(rx);
}
...
fn set_run_fields_no_op_when_run_unknown() {
    ...
    tree.set_run_project_toml_path("nonexistent", Some("/x".into()));
}
