...
use sentinel_daemon::tracked::{CoverageGap, ProcessTree, RunRecord, TreeError};

fn token(pid: u32, pidversion: u32) -> AuditToken {
    AuditToken { val: [0, 0, 0, 0, 0, pid, 0, pidversion] }
}
...
fn tree_05_grandchild_inherits_original_root() {
    ...
    // reparents c2 to launchd). Sentinel's `tracked_root` for c2 is r (the
    ...
    assert_eq!(n.tracked_root, r, "TREE-05: grandchild's tracked_root is the original sentinel-run root");
