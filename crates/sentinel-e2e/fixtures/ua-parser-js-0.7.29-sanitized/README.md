# Synthetic ua-parser-js@0.7.29 mock — Sentinel VAL-01 fixture
...
It exists for **Sentinel's VAL-01 validation test only**. The committed
...
Both this README and the script must be updated together; CODEOWNERS gates
both paths.

## Triple-Layer Safety (per CONTEXT D-02)

1. **Sentinel does its job.** The validation test passes only when
   Sentinel's dylib blocks the postinstall's `connect()` to
...
2. **No real C2.** `c2-sink.test.invalid` is in the IETF-reserved
   `.invalid` TLD — it cannot resolve, ever. Even if Sentinel fails,
...
- **Drive-by PR mutation:** This directory is CODEOWNERS-protected (see
  `.github/CODEOWNERS`). Any change requires explicit maintainer review.
