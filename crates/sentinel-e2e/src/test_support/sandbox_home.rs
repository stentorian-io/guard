...
//!
//! Extracts the tempdir + Library/Logs/Sentinel pattern that lives inline today
...
///   - `Library/Logs/Sentinel/`  (Sentinel's JSONL log directory)
...
pub fn create() -> io::Result<SandboxHome> {
    ...
    let path = home.path();
    std::fs::create_dir_all(path.join("Library/Logs/Sentinel"))?;
