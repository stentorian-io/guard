...
fn disable_list_enable_lifecycle() {
    ...
    let reply: EnableCuratedRuleReply =
        send_tagged(&harness.socket, TAG_ENABLE_CURATED_RULE, &req);
    match reply {
...
fn cli_disable_enable_conflict_rejected() {
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("--enable") || stderr.contains("conflict") || stderr.contains("cannot be used"),
...
fn cli_status_rules_include_built_in_shows_disabled() {
    ...
    // Run `sentinel status rules --include-built-in` and verify output.
    let output = Command::new(&cli)
