...
//!      daemon stderr OR the JSONL sentinel.log — defense-in-depth across two
//!      trust layers (dylib-side filter + daemon-side re-filter, both
//!      mirroring `sentinel_daemon::env_capture`).
//!
//! Wraps the `pm_env_posix_spawn` harness (a small Rust bin under
//! crates/sentinel-e2e/harness/pm_env_posix_spawn). The harness calls
