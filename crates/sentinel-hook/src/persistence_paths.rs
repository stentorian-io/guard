...
mod tests {
    ...
    fn launch_agent_user() {
        assert_eq!(classify(b"/Users/test/Library/LaunchAgents/evil.plist", 14), Some("launch-agent"));
    }

    #[test]
    fn launch_agent_system() {
        assert_eq!(classify(b"/Library/LaunchAgents/evil.plist", 14), Some("launch-agent"));
    }

    #[test]
    fn launch_daemon() {
        assert_eq!(classify(b"/Library/LaunchDaemons/evil.plist", 14), Some("launch-daemon"));
    }
    ...
    #[test]
    fn periodic_scripts() {
        assert_eq!(classify(b"/etc/periodic/daily/evil.sh", 14), Some("periodic-script"));
        assert_eq!(classify(b"/etc/periodic/weekly/mine.sh", 14), Some("periodic-script"));
        assert_eq!(classify(b"/etc/periodic/monthly/cron.sh", 14), Some("periodic-script"));
        assert_eq!(classify(b"/usr/local/etc/periodic/evil", 14), Some("periodic-script"));
    }
    ...
    fn shell_profile_injection() {
        ...
        assert_eq!(classify(b"/Users/test/.bash_profile", 14), Some("shell-profile"));
        ...
        assert_eq!(classify(b"/Users/test/.zprofile", 14), Some("shell-profile"));
    }
    ...
    fn empty_home() {
        assert_eq!(
            classify_persistence_path_with_version(b"/Users/test/Library/LaunchAgents/evil.plist", b"", 14),
            None,
        );
        assert_eq!(
