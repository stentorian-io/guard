...
use sentinel_hook::ipc_client::{
    _clear_daemon_socket_for_test, _set_daemon_socket_for_test, send_env_not_propagated_gap_sync,
    IpcClientError,
};
use sentinel_ipc::frame::{read_frame, write_frame};
use sentinel_ipc::{AuditTokenWire, EnvNotPropagatedGap, EnvNotPropagatedGapAck};
...
fn send_env_not_propagated_gap_sync_err_reply_returns_daemon_rejected() {
    let _lock = SOCKET_TEST_LOCK.lock().unwrap();
    let (_dir, sock_path) =
        spawn_stub_env_gap_daemon(EnvNotPropagatedGapAck::err("untracked peer; ignoring env-not-propagated gap"));
    _set_daemon_socket_for_test(sock_path);
