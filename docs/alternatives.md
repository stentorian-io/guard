...
- **Policy granularity.** EDRs operate at the endpoint level, not the process
  ...
  script" from normal application traffic. Stentorian Guard's policy is scoped to the
...
- **CI and cross-platform.** EDRs protect managed endpoints — they don't run in
  your CI pipeline or on a fresh Linux build box. Stentorian Guard is designed to run
  anywhere you run commands: developer laptops, CI runners, and production
  build systems, across macOS and (soon) Linux.

If your organisation already runs an EDR, Stentorian Guard complements it — the EDR
covers the broad endpoint, Stentorian Guard gives you per-command default-deny
enforcement.
...
- **Per-application, not per-process-tree.** LuLu prompts once for `node` and
  ...
  other. Stentorian Guard's policy is per-destination and per-run, so registry traffic
...
- **Interactive prompts for everything.** LuLu prompts on first connection per
  ...
  transitive dependency's lifecycle script. Stentorian Guard ships curated allowlists
...
- **No threat intelligence.** LuLu doesn't know which destinations are
  associated with known-malicious packages. Stentorian Guard bakes in IOCs from the
  OSV.dev malicious-packages dataset and updates them nightly.

LuLu and Stentorian Guard can coexist — LuLu handles application-level firewall rules,
Stentorian Guard handles per-command enforcement. LuLu is also macOS-only with no CI
story; Stentorian Guard is designed for cross-platform use and headless CI environments.
...
- **Timing.** `npm audit` runs *after* install. Malicious postinstall scripts
  ...
  Stentorian Guard blocks exfiltration *as it happens*, during any command you wrap.
- **Coverage gaps.** Audit databases track CVEs in legitimate packages. A
  ...
  appear in any audit database — but its C2 traffic will hit Stentorian Guard's
  default-deny policy.
- **Different threat model.** Audits catch "this package has a known bug."
  Stentorian Guard catches "this package is trying to phone home." Both matter; they're
  not interchangeable.

Run audits *and* Stentorian Guard — they cover orthogonal risks.
...
- **Pre-install analysis vs. runtime enforcement.** SCA tools analyse package
  ...
  Stentorian Guard intercepts the actual network calls at runtime.
- **Heuristic vs. deterministic.** SCA tools use heuristics to flag "this
  package looks suspicious." Stentorian Guard doesn't guess — if a process tries to
...
- **Zero-day supply-chain attacks.** A brand-new malicious package that no
  scanner has seen yet will sail through SCA tools. Stentorian Guard's default-deny
  policy blocks it on the first connection attempt to an unknown host.

SCA tools are your early warning system. Stentorian Guard is your last line of defence —
...
Use lockfiles. Also use Stentorian Guard.
